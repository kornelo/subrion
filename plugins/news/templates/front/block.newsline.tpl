{if isset($newsline) && $newsline}
	<div class="row-fluid">
		{foreach $newsline as $entry}
			{assign var='num_columns' value=((isset($config.newsline_row_count)) ? $config.newsline_row_count : 3)}
				...
				<div class="ia-items latest-news">
					<div class="media ia-item ia-item-bordered-bottom">
						<div class="media-body">
							{if $entry.image && $config.newsline_display_img}
							...
							<p class="ia-item-date">{$entry.date|date_format:$config.date_format} {lang key='by'} {$entry.fullname}</p>
							<p class="ia-item-body">{$entry.body|strip_tags|truncate:$config.newsline_body_max:'...'} <a href="{$smarty.const.IA_URL}news/{$entry.id}-{$entry.alias}.html">{lang key='continue_reading'}</a></p>
						</div>
					</div>
				</div>
			</div>
