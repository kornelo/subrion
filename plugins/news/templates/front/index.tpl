{if isset($entry)}
	<div class="media ia-item news-entry">
		<p class="ia-item-date">{lang key='posted_on'} {$entry.date|date_format:$config.date_format} {lang key='by'} {$entry.fullname}</p>
			...
			{foreach $news as $entry}
				<div class="media ia-item">
					...
					<div class="media-body">
						...
						<p class="ia-item-date">{lang key='posted_on'} {$entry.date|date_format:$config.date_format} {lang key='by'} {$entry.fullname}</p>
						<div class="ia-item-body">{$entry.body|strip_tags|truncate:$config.news_max:'...'}</div>
					</div>
				</div>
			{/foreach}
		</div>
