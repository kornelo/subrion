...
if ($iaView->getRequestType() == iaView::REQUEST_HTML)
{
	if ($pageAction == iaCore::ACTION_EDIT)
	{
		...
		if (isset($_POST['save']))
		{
			...
			{
				$error = true;
				$messages[] = _t('error_comment_email');
			}
			...
			{
				$iaDb->update($insert);
				$iaView->setMessages(_t('changes_saved'), iaView::SUCCESS);
		...
		$iaView->title(_t('edit_comment'));
		iaBreadcrumb::replaceEnd(_t('edit_comment'), IA_SELF);
